#!/usr/bin/env python3
"""Test ordinary Compose startup with generated state/accounts and no Apple login.

--mode existing starts Navidrome first, then checks the add-on leaves that same
container running. No existing user containers, music or secrets are accessed.
"""
import argparse
import hashlib
import json
...
parser.add_argument('--mode', choices=['full', 'existing'], required=True)
...
with tempfile.TemporaryDirectory(prefix=project) as temp:
    ...
    existing = common + ['-f', str(root / 'existing.json')]
    if args.mode == 'existing':
        ...
        compose = existing + ['-f', str(ROOT / 'compose.existing-navidrome.yml'), '-f', str(isolation)]
    else:
    ...
    try:
        ...
        run(compose + ['up', '-d'])
        if nav_before:
            assert output(compose + ['ps', '-q', 'navidrome']) == nav_before, 'Existing Navidrome was replaced'
        ...
        if args.mode == 'full':
            run(compose + ['down'])  # Retain named volumes, as in normal operation.
            run(compose + ['up', '-d'])
        else:
        ...
        assert len(sub('getTopSongs', artist='Kid Cudi', count=5)['topSongs']['song']) == 5
        if nav_before:
            assert output(compose + ['ps', '-q', 'navidrome']) == nav_before, 'Existing Navidrome changed during restart'
        print(f'PASS ({args.mode}): one-command initialization, catalog/photos/top songs, scoped token, shared ALAC storage, native ranges and persistent restart.')
    finally:
